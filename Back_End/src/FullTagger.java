...
public class FullTagger  {
    ...
    public FullTagger (String filename) throws NoSuchAlgorithmException, IOException, ClassCastException, ClassNotFoundException  {
    	long startTime, endTime;
        startTime = System.nanoTime();
        
        new SortbyStringLength("compound-class.txt");
        new SortbyStringLength("bioact.txt");
        new SortbyStringLength("cell-lines.txt");
        new SortbyStringLength("orgpart.txt");
        new SortbyStringLength("bodypart.txt");
        new SortbyStringLength("prep.txt");
        new SortbyStringLength("illness.txt");
        ...
        if(checkDocument(uniqueID, filename) || true) {
	        ...
	        List<Pattern> bodyPart_patterns = new ReadAndCompilePatterns("bodypart.txt").readFile().compilePatternsInsensitive().getPatterns();
	        List<Pattern> prep_patterns = new ReadAndCompilePatterns("prep.txt").readFile().compilePatternsInsensitive().getPatterns();
	        List<Pattern> illness_patterns = new ReadAndCompilePatterns("illness.txt").readFile().compilePatternsInsensitive().getPatterns();
	
	
	        List<String> bioact_found = new ArrayList<>();
	        List<String> family_found = new ArrayList<>();
	        List<String> species_found = new ArrayList<>();
	        ...
	        List<String> class_found = new ArrayList<>();
	        List<String> loc_found = new ArrayList<>();
	        List<String> bodyPart_found = new ArrayList<>();
	        List<String> prep_found = new ArrayList<>();
	        List<String> illness_found = new ArrayList<>();
	        ...
	        for(Pattern pattern : bioact_patterns) {
	            ...
	            while(matcher.find()) {
	            	if(!bioact_found.contains(matcher.group()))
	        ...
	        System.out.println("finding families");
	        for(Pattern pattern : family_patterns) {
	            Matcher matcher = pattern.matcher(txt);
	            while(matcher.find()) {
	            	if(!family_found.contains(matcher.group()))
	            		family_found.add(matcher.group());
	                Integer n = map.get(matcher.group().trim());
	                n = (n == null) ? 1 : ++n;
	                map.put(matcher.group().trim(), n);
	            }
	            ...
	            while(matcher.find()) {
	                //if(!matcher.group().contains("family")) {
	                    ...
	                    //if(matcher.group().split(" ").length==2) {
	                    	if(!species_found.contains(matcher.group().trim()))
	        ...
	        for(Pattern pattern : orgpart_patterns) {
	            Matcher matcher = pattern.matcher(txt);
	            while(matcher.find()) {
	            	if(!cell_found.contains(matcher.group()))
	        ...
	        //System.err.println("[NER using Stanford NER] Duration: "+ ((double)(endTime1 - startTime1)) / 1000000 + " ms");
	        ...
	        for(String family : family_found) {
	            //System.out.println(bioact);
	            ent.put("family "+ent_idx, family);
	            //txt = txt.replaceAll(bioact, "<bioact>" + bioact + "</bioact>");
	            txt = txt.replaceAll(family, "<< family "+ent_idx+" >>");
	            ent_idx++;
	        }
	        ...
	        */
	        ...
	        java.io.FileWriter fw = new java.io.FileWriter("texts\\"+uniqueID+".xml");
	        fw.write(txt);
	        fw.close();	   
	        
	        fw = new java.io.FileWriter("texts\\"+uniqueID+".txt");
	        fw.write(cleanTxt);
	        fw.close();
        } else {
        	System.err.println("Duplicate document");
        }
