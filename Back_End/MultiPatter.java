...
public class MultiPatter {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, NoSuchAlgorithmException {
        ...
        HashSet<Pattern> bioact_patterns = new ReadAndCompilePatterns("bioact.txt").readFile().compilePatternsInsensitive().getPatterns();

        HashSet<Pattern> genus_patterns = new ReadAndCompilePatterns("genus.txt", "", "\\s(\\b[a-z]*)").readFile().compilePatterns().getPatterns();
        genus_patterns.add(Pattern.compile("[A-Z]\\.\\s([a-z])+"));
        genus_patterns.add(Pattern.compile("\\s([A-Z]{2})\\s"));

        HashSet<Pattern> orgpart_patterns = new ReadAndCompilePatterns("orgpart.txt").readFile().compilePatternsInsensitive().getPatterns();

        HashSet<Pattern> cell_patterns = new ReadAndCompilePatterns("cell-lines.txt").readFile().compilePatterns().getPatterns();

        HashSet<Pattern> aka_patterns = new HashSet<>();

        HashSet<Pattern> class_patterns = new ReadAndCompilePatterns("compound-class.txt").readFile().compilePatternsInsensitive().getPatterns();
